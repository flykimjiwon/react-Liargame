...
// setSub(props.sub)
useEffect(()=>{
  setSub(shuffleArray(props.sub)[0])
},[sub])
...
<h2>고른인원은{props.person}명</h2>
<h2>정한시간은{props.time}초</h2>
<h2>주제는-{sub}-입니다.</h2>
<Button variant="contained" onClick = { ()=>{
  setLiarmodal(!liarmodal)
  if (props.person==="2"||props.person===2){
    setLiar2(choiceLiar(props.person))
  }else if(props.person===3){
    setLiar3(choiceLiar(props.person))
  }else if(props.person===4){
    setLiar4(choiceLiar(props.person))
  }else if(props.person===5){
    setLiar5(choiceLiar(props.person))
  }else if(props.person===6){
    setLiar6(choiceLiar(props.person))
  }
} }>라이어고르기</Button>
{/* {playNum(props.person)} */}
{
  liarmodal === true
  ?   (
    playNum(props.person).map(function(p,key){
      return(
        <div>
        {
          p === 'Liar'
